...
int main()
{
    ...
    while(t--)
    {
        ...
        while(qry--)
        {
            ...
            set<int> col;
            FOR(i, nodes[u].size(), 1) col.insert(nodes[u][i].second);
            // Out(col.size());
            for(int i=*col.begin(); !col.empty(); col.erase(col.begin()), i=*col.begin())
            {
                // cout<< i<< ",\t";
                queue<int> q;
                bitset<100> visited;
                ...
                while(!q.empty())
                {
                    ...
                    FOR(j, nodes[cur].size(), 1)
                    {
                        if(nodes[cur][j].second!=i) continue;
                        q.push(nodes[cur][j].first);
                    }
                }
                if(visited[v]) co++;
            }
