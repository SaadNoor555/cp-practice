...
void build(int idx, int l, int r)
{
    ...
    seg[idx]= seg[2*idx]+seg[2*idx];
    rng[idx]= {l, r};
}

void update(int idx, const int sm,int sl, int sr, int l, int r)
{
    // cout<< sl<< " "<< sr<< " "<< l<< " "<< r<< "\n";
    if(sl==l && sr==r)
    {
...
{
    if(sl>r || sr<l || sl>sr || l>r)
        return 0;

    if(sl==l && sr==r)
    {
        // cout<< idx<< " "<< (seg[idx]+add[idx]*(rng[idx].second-rng[idx].first+1))<< "\n";
        return (seg[idx]+add[idx]*(rng[idx].second-rng[idx].first+1));
    }
    ...
    if(add[idx])
    {
        add[2*idx]+=add[idx];
        add[2*idx+1]+=add[idx];
        seg[idx]+=(add[idx]*(rng[idx].second-rng[idx].first+1));
        add[idx]= 0;
    }
    int mid= (l+r)/2;
    int n1= query(2*idx, sl, min(sr, mid), l, mid);
    int n2= query(2*idx+1, max(sl, mid+1), sr, mid+1, r);
