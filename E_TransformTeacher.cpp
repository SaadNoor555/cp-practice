...
ll minx(int i)
{
    ll a1= lower_bound(w.begin(), w.end(), h[i*2])-w.begin();
    
    return min(abs(h[i*2]-w[a1]), a1? abs(h[i*2]-w[a1-1]) : INT32_MAX);
}
...
int main()
{
    ...
    FOR(x, 1, t+1, 1)
    {
        ...
        FOR(i, 0, n-1, 1)
            dif.PB(abs(h[i]-h[i+1]));

        Edif.PB(0);
        FOR(i, 0, dif.size(), 2)
            Edif.PB(*(Edif.end()-1)+dif[i]);

        Odif.PB(0);
        FOR(i, 1, dif.size(), 2)
            Odif.PB(*(Odif.end()-1)+dif[i]);
        ...
        FOR(i, 0, n/2+1, 1)
        {
            ll temp= minx(i)+Edif[i]+Odif[n/2]-Odif[i];
            // cout<< minx(i)<< " "<< Edif[i]<< " "<< Odif[n/2]<< " "<< Odif[i]<< "\n";
            // Out(temp);
            mnm= min(mnm, temp);
        }
