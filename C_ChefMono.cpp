    ...
    while(t--)
    {
        ...
        for(int idx= 0; idx<n; idx++)
        {
            ...
            {           
                ...
                for(int time= idx+1; time<40000; time++)
                    dp[idx][time]= min(abs(time-d[idx])+dp[idx-1][time-1], dp[idx][time-1]);
            }

            else 
                for(int time= idx+1; time<40000; time++)
                    dp[idx][time]= min(abs(time-d[idx]), dp[idx][time-1]);
        }
