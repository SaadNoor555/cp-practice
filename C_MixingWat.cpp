...
double calD(double an, double h, double c, double t)
{
    double tm= (h+(h+c)*((an-1)/2))/an;
	// Out(abs(t-tm));
	return abs(t-tm);
}
...
int main()
{
    ...
    while(t--)
    {
		double h, c, t;
		ll an;
        double avg=0, tmp=0, sum=0, mn=INT64_MAX;
        cin>> h>> c>> t;
        if((h+c)/2>=t) an= 2;
        else
        {
            an=(t-c)/(2*t-h-c);
			// Out(an);
            if(calD(2*an+1, h, c, t)<calD(2*an-1, h, c, t)) an= 2*an+1;
			else an= 2*an-1;
        }
        Out(an);
