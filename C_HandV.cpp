...
{
    int c=0;
    rep(i, h) c+=count(s[i].begin(), s[i].end(), '#');
    return c;
}
...
void match(string *s, int mask)
{
    ll tr= 1;
    for(int i=0; tr<=mask; i++)
    {
        if(mask&tr)
            rep(j, w) s[i][j]='.';
...
void foo(int cnt)
{
    ...
    string s[h], cl[h], tmp[h];
    rep(i, h) cin>> s[i], cl[i]=s[i];
    
    int mxh=pow(h), mxw=pow(w);
