...
string s;
int pal(int str, int fin)
{
    if(fin<=str) return 0;
    if(dp[str][fin]!=-1) return dp[str][fin];

    if(s[str]==s[fin]) dp[str][fin]= pal(str+1, fin-1);
    else dp[str][fin]= 1+min(pal(str+1, fin), pal(str, fin-1));
