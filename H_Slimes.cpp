...
int main()
{
    ...
    while(t--)
    {
        ...
        {
            // int mindex= 0;
            int mindex= min_element(dif.begin(), dif.end())-dif.begin();
            ll p= dif[mindex];
            an+=p;
            // Out(mindex);
            dif.erase(dif.begin()+mindex);
            a.erase(a.begin()+mindex+1);
            a[mindex]=p;
            if(mindex)dif[mindex-1]= a[mindex]+a[mindex-1];
            if(mindex+1<a.size()) dif[mindex]= a[mindex]+a[mindex+1];
            // break;
        }
        Out(an);
    }
