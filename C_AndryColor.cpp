...
int main()
{
    ...
    while(t--)
    {
        int n, u, v, c=1, maxm=1;
        cin>> n;
        int color[n];
        vector<vint> nodes(n);
        FOR(i, n, 1) color[i]= 200009;
        FOR(i, n-1, 1)
        {
            cin>> u>> v;
            nodes[--u].PB(--v);
        ...
        while(!q.empty())
        {
            piin p= q.front();
            q.pop();
            FOR(i, nodes[p.first].size(), 1)
            {
                if(c==color[p.first] || c==p.second) c++;
                if(c==color[p.first] || c==p.second) c++;
                if(color[nodes[p.first][i]]==200009)
                {
                    color[nodes[p.first][i]]= c;
                    maxm= max(maxm, c++);
                    q.push({nodes[p.first][i], color[p.first]});
                }
            }
            c=1;
        }
