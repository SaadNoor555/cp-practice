...
int main()
{
    ...
    while(t--)
    {
        int n;
        cin>> n;
        int a[n], ans[n+1], minm= INT32_MAX;
        bool g=0;
        int md[n+1];
        InAr(a, n);
        vint pos[n+1];

        FOR(i, n+1, 1) pos[i].PB(-1), md[i]=0, ans[i]=-1;
        FOR(i, n, 1)
            pos[a[i]].PB(i);
        FOR(i, n+1, 1) pos[i].PB(n);

        for(int i=1; i<n+1; i++)
            if(pos[i].size()>2)
                for(int j=1; j<pos[i].size(); j++)
                    md[i]= max(md[i], pos[i][j]-pos[i][j-1]);
        ...
        {
            if(!g && ans[i]!=-1) g=1;
            if(g && ans[i]!=-1) minm= min(ans[i], minm);
            if(!g) cout<< -1<< " ";
            else
            {
                if(ans[i]==-1) cout<< minm<< " ";
                else cout<< min(minm, ans[i])<< " ";
            }
        }
        cout<< "\n";
