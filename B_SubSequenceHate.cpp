...
int main()
{
    ...
    while(t--)
    {
        ...
        // Out(ct.size());
        int psum[ct.size()+2];
        if(ct.size()==1) psum[0]=ct[0];
        else for(int i=ct.size()-1; i>=0; i--)
        {
            if(i>=ct.size()-2)psum[i]= ct[i];
            else psum[i]=(ct[i]+psum[i+2]);
        }
        psum[ct.size()]= 0;
        psum[ct.size()+1]= 0;
        c=0;

        // OutAr(psum, ct.size()+2);
