...
int main()
{
    ...
    while(t--)
    {
        ...
        sort(cords.begin(), cords.end(), greater<int>());
        int n;
        cin>> n;
        ll tab[n][6];
        FOR(i, n, 1)
        ...
        FOR(i, n, 1)
        {
            ...
            FOR(j, 6, 1)
            {
                FOR(k, 6, 1)
                {
                    if(tab[i][j]<pas[k][0])
                    {
                        if(pas[k][1]-tab[i][j]<=cur[j][2])
                        {
                            cur[j][0]= tab[i][j];
                            cur[j][1]= pas[k][1];
                            cur[j][2]= cur[j][1]-cur[j][0];
                        }
                    }
                    else if(tab[i][j]>pas[k][1])
                    {
                        if(tab[i][j]-pas[k][0]<=cur[j][2])
                        {
                            cur[j][0]= pas[k][0];
                            cur[j][1]= tab[i][j];
                            cur[j][2]= cur[j][1]-cur[j][0];
