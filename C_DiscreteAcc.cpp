...
int main()
{
    ...
    while(t--)
    {
        ...
        double str, end, curs= 0,cure=l, v1=1, v2=1;
        for(int i=1, str=0, end=n-1; i<=n; i++)
        {
            an+=min((a[str]-curs)/v1 , (cure-a[end])/v2);
            if((a[str]-curs)/v1 < (cure-a[end])/v2)
            {
                cure-= ((a[str]-curs)/v1)*v2;
                v1++;
                curs= a[str++];
            }
            else if((a[str]-curs)/v1 > (cure-a[end])/v2)
            {
                curs+= ((cure-a[end])/v2)*v1;
                v2++;
                cure=a[end--];
            }
            else
            {
                v1++, v2++;
                cure= a[end--], curs= a[str++];
                i++;
            }
        }
        an+=abs(cure-curs)/(v1+v2);
        cout<< an<< "\n";
    }
}