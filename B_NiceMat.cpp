...
int main()
{
    ...
    while(t--)
    {
        int n, m;
        cin>> n>> m;
        ll a[101][101], s=0, an1=0, an2=0, an=0;
        FOR(i, n, 1)
            FOR(j, m, 1)
                cin>> a[i][j];
        ...
        FOR(i, n/2+n%2, 1)
            FOR(j, m/2+m%2, 1)
            {
                ...
                s/=4;
                // Out(s);
                an1=abs(s-a[i][j])+abs(s-a[i][m-j-1])+abs(s-a[n-i-1][j])+abs(s-a[n-i-1][m-j-1]);
                an2=abs(s-a[i][j]+1)+abs(s-a[i][m-j-1]+1)+abs(s-a[n-i-1][j]+1)+abs(s-a[n-i-1][m-j-1]+1);
                if(i==n-i-1) an1/=2, an2/=2;
                if(j==m-j-1) an1/=2, an2/=2;
                an+=min(an1, an2);
                // vis[i][j]=1, vis[i][m-j-1]=1, vis[n-i-1][j]=1, vis[n-i-1][m-j-1]= 1;
            }
