...
int r, n;
// int mem[501][501][1001];
int mem[100001];
int cel[100001][3];

int dp(int x, int y, int t, int idx)
{
    int ans=0;
    if(idx==n) return 0;
    if(mem[idx]!=-1) return mem[idx];
    if(abs(cel[idx][0]-x)+abs(cel[idx][1]-y)<=cel[idx][2]-t)
        ans= max(1+dp(cel[idx][0], cel[idx][1], cel[idx][2], idx+1), ans);
    ans= max(ans, dp(x, y, t, idx+1));
    mem[idx]= ans;
    return ans;
}
