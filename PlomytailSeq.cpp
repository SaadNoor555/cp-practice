...
int main()
{
    ...
    while(t--)
    {
        ...
        FOR(i, n, 1)
        {
            p[i]=upper_bound(pref.begin(), pref.end(), a[i])-pref.begin();
            pref.insert(pref.begin()+p[i], a[i]);
        }
        pref.clear();
        for(int i=n-1; i>=0; i--)
        {
            s[i]=upper_bound(pref.begin(), pref.end(), a[i])-pref.begin();
            pref.insert(pref.begin()+s[i], a[i]);
        }
        int maxm= 0;
        FOR(i, n, 1)
            cout<<p[i]<<" "<<s[i]<<"\n";
