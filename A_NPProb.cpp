...
vint color(100001, WHITE);

bool BFS(int n, vector<vint>graph)
{
    FOR(i, n, 1)
    {
        if(color[i]) continue;
        ...
        while(!q.empty())
        {
            ...
            int cr= q.front();
            q.pop();
            for(int j=0; j<graph[cr].size(); j++)
            {
            // cout<< graph[cr][j]<<"\n";
                if(color[graph[cr][j]]==WHITE) color[graph[cr][j]]=color[cr]%2+1, q.push(graph[cr][j]);
                else if(color[graph[cr][j]]==color[cr]) return false;
            }
        }
    }
    return true;
