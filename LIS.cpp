...
int LIS(int idx, int ref)
{
    if(idx==n) return 0;
    if(ref!=-1 && mem[idx][ref]!=-1) return mem[idx][ref];
    int p1=0, p2=0;
    if(ref==-1 || seq[idx]>seq[ref]) p1=LIS(idx+1, idx)+1;
    p2=LIS(idx+1, ref);
    // if(!ref) p2++;
    if(ref!=-1)
    {
        mem[idx][ref]= max(p1, p2);
        return mem[idx][ref];
    }
    return max(p1, p2);
}
